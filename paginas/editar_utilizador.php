<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['nome_proprio'], $_POST['nome_utilizador'], $_POST['tipo_utilizador'], $_POST['id_carteira'])) {
            ...
            $sql_bilhete = "UPDATE utilizador SET nome_proprio = ?, nome_utilizador = ?, tipo_utilizador = ?, id_carteira = ? WHERE id = ?";
            $stmt_bilhete = $conn->prepare($sql_bilhete);

            if ($stmt_bilhete) {
                $stmt_bilhete->bind_param("ssiii", $novo_nome_proprio, $novo_nome_utilizador, $novo_tipo_utilizador, $novo_id_carteira, $id_utilizador);
